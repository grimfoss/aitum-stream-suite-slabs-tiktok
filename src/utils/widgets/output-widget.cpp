...
#include <src/streamlabs/api-client.hpp>
...
StreamlabsApiClient *OutputWidget::streamlabsClient()
{
...
void OutputWidget::output_stop(void *data, calldata_t *calldata)
{
	...
	if (this_->output) {
		...
		QString streamId = this_->m_streamlabsStreamId;
		this_->m_streamlabsStreamId.clear();
		QString streamlabsToken = this_->settings
						? QString::fromUtf8(obs_data_get_string(this_->settings, "streamlabs_token"))
						: QString();
		QString outputName = this_->settings
					 ? QString::fromUtf8(obs_data_get_string(this_->settings, "name"))
					 : QString();

		QMetaObject::invokeMethod(
			this_->outputButton,
			[this_, last_error, code, streamId, streamlabsToken, outputName] {
				// End Steamlabs TikTok stream on main thread
				if (!streamId.isEmpty() && !streamlabsToken.isEmpty()) {
					this_->streamlabsClient()->endStream(streamlabsToken, streamId);
				}
				// Clear persisted stream ID regardless
...
bool OutputWidget::StartOutput(bool automated)
{
	...
	if (is_record) {
		...
		auto key_source = obs_data_get_string(settings, "key_source");
		bool isStreamlabsTikTok = (key_source && strcmp(key_source, "streamlabs_tiktok") == 0);

		QString streamlabsServer;
		QString streamlabsKey;

		if (isStreamlabsTikTok) {
			auto token = QString::fromUtf8(obs_data_get_string(settings, "streamlabs_token"));
			auto title = QString::fromUtf8(obs_data_get_string(settings, "stream_title"));
			auto gameMaskId = QString::fromUtf8(obs_data_get_string(settings, "game_mask_id"));
			auto audienceType = QString::fromUtf8(obs_data_get_string(settings, "audience_type"));

			if (token.isEmpty()) {
				blog(LOG_WARNING, "[Aitum Stream Suite] Streamlabs TikTok: no token");
				if (!automated)
					QMessageBox::warning(this, QString::fromUtf8("Streamlabs TikTok"),
							     QString::fromUtf8("Not logged in to Streamlabs."));
				return false;
			}

			auto info = streamlabsClient()->startStream(token, title, gameMaskId, audienceType);
			if (info.rtmpUrl.isEmpty() || info.streamKey.isEmpty()) {
				blog(LOG_WARNING, "[Aitum Stream Suite] Streamlabs TikTok: startStream failed: %s",
				     streamlabsClient()->lastError().toUtf8().constData());
				if (!automated)
					QMessageBox::warning(this, QString::fromUtf8("Streamlabs TikTok Error"),
							     QString::fromUtf8("Failed to start TikTok stream:\n%1")
								     .arg(streamlabsClient()->lastError()));
				return false;
			}

			m_streamlabsStreamId = info.streamId;
			{
				QSettings qs(QStringLiteral("Aitum"), QStringLiteral("StreamSuite"));
				qs.setValue(
					QStringLiteral("streamlabs_stream_id/%1")
						.arg(QString::fromUtf8(
							obs_data_get_string(settings, "name"))),
					info.streamId);
			}
			streamlabsServer = info.rtmpUrl;
			streamlabsKey = info.streamKey;
		}

		QByteArray streamlabsServerUtf8;
		QByteArray streamlabsKeyUtf8;
		if (isStreamlabsTikTok) {
			streamlabsServerUtf8 = streamlabsServer.toUtf8();
			streamlabsKeyUtf8 = streamlabsKey.toUtf8();
		}

		const char *server = isStreamlabsTikTok
					    ? streamlabsServerUtf8.constData()
		...
		const char *key = isStreamlabsTikTok
					  ? streamlabsKeyUtf8.constData()
