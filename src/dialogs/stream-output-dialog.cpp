...
#include "../streamlabs/api-client.hpp"
...
bool isStreamlabsTikTokOutput(const QString &keySource)
{
	return keySource == QStringLiteral("streamlabs_tiktok");
}
...
void StreamOutputDialog::resetOutputs()
{
	...
	outputKey = QString("");
	outputKeySource = QString("");
	outputStreamlabsToken = QString("");
	outputTitle = QString("");
	outputGameMaskId = QString("");
	outputAudienceType = QString("0");
}
...
void StreamOutputDialog::validateOutputs(QPushButton *confirmButton)
{
	...
	if (isStreamlabsTikTokOutput(outputKeySource)) {
		confirmButton->setEnabled(!outputStreamlabsToken.isEmpty() && !outputTitle.isEmpty());
	} else {
		if (outputServer.isEmpty() || outputKey.isEmpty()) {
			confirmButton->setEnabled(false);
		} else {
...
StreamOutputDialog::StreamOutputDialog(QDialog *parent, QStringList _otherNames) : QDialog(parent), otherNames(_otherNames)
{
	...
	stackedWidget->addWidget(WizardInfoStreamlabsTikTok());
...
// Edit mode
StreamOutputDialog::StreamOutputDialog(QDialog *parent, QString name, QString server, QString key,
				       QStringList _otherNames, const QString &keySource,
				       const QString &token, const QString &title,
				       const QString &gameMaskId, const QString &audienceType)
	: QDialog(parent),
	  otherNames(_otherNames)
{
	...
	outputServer = server;
	outputKey = key;
	outputKeySource = keySource;
	outputStreamlabsToken = token;
	outputTitle = title;
	outputGameMaskId = gameMaskId;
	outputAudienceType = audienceType;
	...
	// Add the appropriate page to the layout based upon the server url or key source
	if (isStreamlabsTikTokOutput(outputKeySource)) {
		layout->addWidget(WizardInfoStreamlabsTikTok(true));
	} else if (isTwitchServer(outputServer)) { // twitch
...
QWidget *StreamOutputDialog::WizardServicePage()
{
	...
	rowThree->addWidget(selectionButton("Streamlabs TikTok", platformIconStreamlabs, 9));
...
StreamlabsApiClient *StreamOutputDialog::streamlabsClient()
{
	if (!m_streamlabsClient) {
		m_streamlabsClient = new StreamlabsApiClient(this);
	}
	return m_streamlabsClient;
}

void StreamOutputDialog::onStreamlabsLoginClicked(QPushButton *loginBtn, QPushButton *confirmBtn,
						  QLabel *usernameLabel, QLabel *statusLabel,
						  QLineEdit *titleField, QComboBox *categoryCombo,
						  QStackedWidget *stack)
{
	loginBtn->setEnabled(false);
	loginBtn->setText(QString::fromUtf8("Opening browser..."));

	QTimer::singleShot(100, [this, loginBtn, confirmBtn, usernameLabel, statusLabel,
				 titleField, categoryCombo, stack]() {
		QString token = streamlabsClient()->waitForToken();

		if (token.isEmpty()) {
			loginBtn->setEnabled(true);
			loginBtn->setText(QString::fromUtf8("Login with Streamlabs"));
			QMessageBox::warning(this, QString::fromUtf8("Login Failed"),
					     QString::fromUtf8("Streamlabs login failed: %1\nPlease try again.")
						     .arg(streamlabsClient()->lastError()));
			return;
		}

		StreamlabsAccountInfo info = streamlabsClient()->getAccountInfo(token);

		outputStreamlabsToken = token;
		usernameLabel->setText(info.username.isEmpty()
					       ? QString::fromUtf8("Unknown")
					       : info.username);

		statusLabel->setText(info.canBeLive
					     ? QString::fromUtf8("Ready to go live")
					     : info.applicationStatus.isEmpty()
						       ? QString::fromUtf8("Cannot go live")
						       : info.applicationStatus);

		stack->setCurrentIndex(1);
		loginBtn->setText(QString::fromUtf8("Logged in"));
		loginBtn->setEnabled(false);

		validateOutputs(confirmBtn);
	});
}

QWidget *StreamOutputDialog::WizardInfoStreamlabsTikTok(bool edit)
{
	auto page = new QWidget(this);
	page->setStyleSheet("padding: 0px; margin: 0px;");
	...
	// --- Auth section (always visible on first run, hidden after login) ---
	auto authPage = new QWidget;
	auto authLayout = new QVBoxLayout;
	authLayout->setContentsMargins(0, 0, 0, 0);
	authLayout->setSpacing(8);

	auto loginButton = new QPushButton(QString::fromUtf8("Login with Streamlabs"));
	loginButton->setMinimumHeight(36);
	loginButton->setStyleSheet("font-weight: bold; padding: 8px;");
	authLayout->addWidget(loginButton);

	auto loginInfo = new QLabel(QString::fromUtf8(
		"Click the button above to open your browser and log in with your Streamlabs account.\n"
		"You must have TikTok LIVE access enabled on your Streamlabs account."));
	loginInfo->setWordWrap(true);
	loginInfo->setStyleSheet("color: gray; font-size: 11px;");
	authLayout->addWidget(loginInfo);
	authPage->setLayout(authLayout);

	// --- Config section (visible after login) ---
	...
	configLayout->setLabelAlignment(Qt::AlignRight | Qt::AlignTrailing | Qt::AlignVCenter);
	configLayout->setSpacing(6);

	auto usernameLabel = new QLabel(QString::fromUtf8("-"));
	usernameLabel->setStyleSheet("font-weight: bold;");
	configLayout->addRow(QString::fromUtf8("Account:"), usernameLabel);

	auto statusLabel = new QLabel(QString::fromUtf8("-"));
	statusLabel->setStyleSheet("font-weight: bold;");
	configLayout->addRow(QString::fromUtf8("Status:"), statusLabel);

	auto outputNameField = generateOutputNameField("StreamlabsTikTokOutput", confirmButton, edit);
	configLayout->addRow(generateFormLabel("OutputName"), outputNameField);

	auto titleField = new QLineEdit;
	titleField->setPlaceholderText(QString::fromUtf8("Enter stream title..."));
	if (edit)
		titleField->setText(outputTitle);
	connect(titleField, &QLineEdit::textEdited, [this, titleField, confirmButton] {
		outputTitle = titleField->text();
		validateOutputs(confirmButton);
	});
	configLayout->addRow(generateFormLabel("StreamlabsTikTokTitle"), titleField);

	auto categoryCombo = new QComboBox;
	categoryCombo->setEditable(true);
	categoryCombo->setMinimumHeight(24);
	categoryCombo->setPlaceholderText(QString::fromUtf8("Type to search categories..."));
	categoryCombo->setInsertPolicy(QComboBox::NoInsert);

	// Debounced search as you type
	QTimer *searchTimer = new QTimer(categoryCombo);
	searchTimer->setSingleShot(true);
	connect(categoryCombo, &QComboBox::editTextChanged, [this, categoryCombo, searchTimer]() {
		searchTimer->stop();
		searchTimer->start(350);
	});
	connect(searchTimer, &QTimer::timeout, [this, categoryCombo]() {
		QString query = categoryCombo->currentText().trimmed();
		if (query.length() < 2 || outputStreamlabsToken.isEmpty())
			return;

		auto categories = streamlabsClient()->searchCategories(outputStreamlabsToken, query);
		categoryCombo->blockSignals(true);
		QString currentText = categoryCombo->currentText();
		categoryCombo->clear();
		for (const auto &cat : categories)
			categoryCombo->addItem(cat.fullName, cat.gameMaskId);
		categoryCombo->setEditText(currentText);
		categoryCombo->blockSignals(false);
		// Show dropdown with results
		if (categories.size() > 0)
			categoryCombo->showPopup();
	});
	// Store game mask when user selects from dropdown
	connect(categoryCombo, &QComboBox::currentIndexChanged, [this, categoryCombo, confirmButton](int index) {
		if (index >= 0) {
			outputGameMaskId = categoryCombo->itemData(index).toString();
		}
		validateOutputs(confirmButton);
	});
	configLayout->addRow(QString::fromUtf8("Category:"), categoryCombo);

	auto matureCheck = new QCheckBox(QString::fromUtf8("Enable mature content"));
	if (edit)
		matureCheck->setChecked(outputAudienceType == QStringLiteral("1"));
	connect(matureCheck, &QCheckBox::toggled, [this](bool checked) {
		outputAudienceType = checked ? QStringLiteral("1") : QStringLiteral("0");
	});
	configLayout->addRow(QString::fromUtf8(""), matureCheck);

	auto endStreamBtn = new QPushButton(QString::fromUtf8("End TikTok Stream"));
	endStreamBtn->setMinimumHeight(36);
	endStreamBtn->setStyleSheet("font-weight: bold; padding: 8px; color: #e04040;");
	endStreamBtn->setVisible(false);
	configLayout->addRow(QString::fromUtf8(""), endStreamBtn);
	...
	if (edit && !outputStreamlabsToken.isEmpty()) {
		...
		// Check for active stream ID to show End Stream button
		QSettings qs(QStringLiteral("Aitum"), QStringLiteral("StreamSuite"));
		QString activeStreamId = qs.value(QStringLiteral("streamlabs_stream_id/%1").arg(outputName)).toString();
		if (!activeStreamId.isEmpty() && !outputStreamlabsToken.isEmpty()) {
			endStreamBtn->setVisible(true);
			connect(endStreamBtn, &QPushButton::clicked, [this, activeStreamId, endStreamBtn] {
				bool ok = streamlabsClient()->endStream(outputStreamlabsToken, activeStreamId);
				if (ok) {
					QMessageBox::information(this, QString::fromUtf8("Stream Ended"),
								 QString::fromUtf8("TikTok stream ended successfully."));
					QSettings qs(QStringLiteral("Aitum"), QStringLiteral("StreamSuite"));
					qs.remove(QStringLiteral("streamlabs_stream_id/%1").arg(outputName));
					endStreamBtn->setVisible(false);
				} else {
					QMessageBox::warning(this, QString::fromUtf8("Error"),
							     QString::fromUtf8("Failed to end stream:\n%1")
								     .arg(streamlabsClient()->lastError()));
				}
			});
		}
	...
	connect(loginButton, &QPushButton::clicked, [this, loginButton, confirmBtn = confirmButton,
						    ...
						    categoryCombo, stack]() {
		onStreamlabsLoginClicked(loginButton, confirmBtn, usernameLabel, statusLabel,
					 titleField, categoryCombo, stack);
		stack->setCurrentIndex(1);
	});
	...
	connect(confirmButton, &QPushButton::clicked, [this] {
		outputKeySource = QStringLiteral("streamlabs_tiktok");
		acceptOutputs();
	});
	...
	if (!edit) {
		connect(stackedWidget, &QStackedWidget::currentChanged,
			[this, outputNameField, titleField, categoryCombo, confirmButton] {
